...
The SQL part and the Mongo part created in Task 2 have been expanded and implemented in the Task 3 API, in the FastAPI framework. It is in `crud_api/` with packages for the SQL API and the Mongo API, and a common `utils.py` for the use of the same calendar features (year, month, hour, day_of_week, is_weekend, season) as when the existing data was bulk-loaded in Task 2, so that the records generated from the API match with the existing rows/documents. All databases are exposed at the same resource shape, `/api/sql/readings` and `/api/mongo/readings` with the same CRUD (`POST`, `GET` by id, `PUT`, `DELETE`) and the two necessary time-series resource shapes, `GET /latest` and `GET /range?start=...&end=...`. These interactive docs can be generated by FastAPI automatically at `/docs`. The MongoDB endpoints are organized the same way as the SQL endpoints, and share the same route/schema code; initially I was unable to test them in a "real" environment because of the difficulty in getting MongoDB working locally, so I added an end-to-end test suite (crud_api/tests/test_mongo_api.py) that tests the same flow as the SQL endpoints against mongomock, replacing a real database; all 13 cases (create, get by id, latest, range, update, delete, and the invalid/missing-id error paths) pass.
